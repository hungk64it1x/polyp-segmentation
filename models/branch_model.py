...
class Reverse_FAM(nn.Module):
    ...
    def forward(self, feat_l, feat_s):
        '''
        feat_l: out - 128
        feat_s: feat - 512 320 128
        '''
        
        # out = align_module(out, feat)
        feat_up = feat_s
        if feat_l.shape[2:] != feat_s.shape[2:]:
            feat_up = F.interpolate(feat_s, size=feat_l.shape[2:], mode='bilinear', align_corners=False)
        feat_arm = self.lateral_conv(feat_up) #128
        offset = self.offset(torch.cat([feat_l, feat_arm*2], dim=1))

        feat_align = F.relu(self.dcpack_l2(feat_arm, offset))
        return feat_align + feat_l
    
class FaPNHead(nn.Module):
    def __init__(self, in_channels, channel=128, num_classes=19):
        super().__init__()
        in_channels = in_channels[::-1] # 512 320 128
        self.align_modules = nn.ModuleList([ConvModule(in_channels[0], channel, 1)])
        self.reverse_align_modules = nn.ModuleList([])
        self.output_convs = nn.ModuleList([])

        for ch in in_channels[1:]:
            self.align_modules.append(FAM(ch, channel))
            self.reverse_align_modules.append(Reverse_FAM(ch, channel))
    ...
    def forward(self, features) -> Tensor:
        features = features[::-1]
        out = self.align_modules[0](features[0]) # 512 - 128
        
        for feat, align_module, reverse_align_module, output_conv in zip(features[1:], self.align_modules[1:], self.reverse_align_modules, self.output_convs):
            out = align_module(feat, out) # align 1
            
            out = output_conv(out) #Pi
            # print(feat.shape) # 320
            # print(out.shape) # 128
            # Reversed FAM
            out = reverse_align_module(out, feat)
            out = output_conv(out)
...
class CustomModel(BaseModel):
    def __init__(self, backbone: str='MiT-B3', decode: str='UPerHead', num_classes: int=1, pretrained=None) -> None:
